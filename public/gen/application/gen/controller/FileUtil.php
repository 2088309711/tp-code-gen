...
class FileUtil
{
    ...
    public static function copyDir($fromDir, $toDir, $overWrite = false)
    {
        ...
        $toDir = substr($toDir, -1) === '/' ? $toDir : $toDir . '/';

        $fromDir = str_replace('\\', '/', $fromDir);
        $fromDir = substr($fromDir, -1) === '/' ? $fromDir : $fromDir . '/';
        ...
        if (!file_exists($toDir)) {
            FileUtil::createDir($toDir);
        }

        $dirHandle = opendir($fromDir);
        while (false !== ($file = readdir($dirHandle))) {
            if ($file == '.' || $file == '..') {
                continue;
            }
            if (!is_dir($fromDir . $file)) {
                FileUtil:: copyFile($fromDir . $file, $toDir . $file, $overWrite);
            } else {
                FileUtil:: copyDir($fromDir . $file, $toDir . $file, $overWrite);
            }
        }
