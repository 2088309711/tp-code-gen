...
class ServerList extends Controller{
	...
	public function _initialize(){	//初始化
		$this->model = new \app\index\model\ServerList;
	}
	    //默认入口
	...
	public function add(){
		if($this->request->isPost()){
			...
			$result = $this->validate($data, 'ServerList.add');
			if ($result !== true) {
				$this->error($result);
			}
	...
	public function edit(){
		if($this->request->isPost()){
			...
			$result = $this->validate($data, 'ServerList.edit');
			if ($result !== true) {
				$this->error($result);
			}
			...
			$result = $this->validate($data, 'ServerList.ck_id');
			if ($result !== true) {
				$this->error($result);
			}
