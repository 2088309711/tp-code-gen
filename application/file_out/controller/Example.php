...
class Example extends Controller{
	...
	public function _initialize(){	//初始化
		$this->model = new \app\index\model\Example;
	}
	    //默认入口
	...
	public function edit(){
		if($this->request->isPost()){
			...
			$result = $this->validate($data, 'Example.edit');
			if ($result !== true) {
				$this->error($result);
			}
			...
			$result = $this->validate($data, 'Example.ck_id');
			if ($result !== true) {
				$this->error($result);
			}


			$example = $this->model->info($this->request);
	...
	public function lists(){
		$exampleList = $this->model->lists($this->request, 12);	
		$this->assign('exampleList', $exampleList);
		return $this->fetch();
	}
}
