...
class CorpusGroup extends Controller{
	protected $model = null;

	public function _initialize(){	//初始化
		$this->model = new \app\index\model\CorpusGroup;
	}
	    //默认入口
	...
	public function edit(){
		if($this->request->isPost()){
			...
			$result = $this->validate($data, 'CorpusGroup.edit');
			if ($result !== true) {
				$this->error($result);
			}
			...
			$corpus_group = $this->model->info($this->request);
			$this->assign('corpus_group', $corpus_group);
			return $this->fetch();
		}
	}
		//删除
	...
	public function lists(){
		$corpus_groupList = $this->model->lists($this->request, 12);	
		$this->assign('corpus_groupList', $corpus_groupList);
		return $this->fetch();
	}
}
