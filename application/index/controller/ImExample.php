...
class ImExample extends Controller{
	protected $model = null;

	public function _initialize(){	//初始化
		$this->model = new \app\index\model\ImExample;
	}
	    //默认入口
	...
	public function edit(){
		if($this->request->isPost()){
			...
			$im_example = $this->model->info($this->request);
			$this->assign('im_example', $im_example);
			return $this->fetch();
		}
	}
		//删除
	...
	public function lists(){
		$im_exampleList = $this->model->lists($this->request, 12);	
		$this->assign('im_exampleList', $im_exampleList);
		return $this->fetch();
	}
}
